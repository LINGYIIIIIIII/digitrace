...
fn main() {
    #[cfg(windows)]
    lifecycle::maybe_takeover_old_version();

    tauri::Builder::default()
        .plugin(tauri_plugin_single_instance::init(|app, _args, _cwd| {
            // 新版双击启动时：单实例唤醒本实例，统一走生命周期回调。
            lifecycle::handle_second_instance(app);
        }))
        .setup(|app| {
            ...
            signals::clear_stale_pending_takeover();
            // 自启（--tray）必须静默：清理残留的「显示窗口」请求，避免登录时误弹窗口。
            if from_autostart {
                signals::clear_stale_show_request();
            }
            // 跨权限接管兜底：轮询新版写入的「待切换」标记并弹窗询问。
            lifecycle::spawn_takeover_poller(app.handle().clone());
            ...
            lifecycle::start_working_set_trimmer();
