...
use windows::Win32::UI::WindowsAndMessaging::{
    DispatchMessageW, EVENT_OBJECT_NAMECHANGE, EVENT_SYSTEM_FOREGROUND, GetMessageW, MSG,
    ...
    I: IdleDetector + 'static,
{
    ...
    thread::spawn(move || {
        ...
        loop {
            // 采样间隔/空闲阈值实时从配置读取：设置页保存后立即生效（无需重启）。
            // 传入的 poll_interval 作为兜底（配置异常为 0 时使用）。
            let cfg = crate::config::AppConfig::load();
            let poll_interval = if cfg.poll_interval_ms > 0 {
                Duration::from_millis(cfg.poll_interval_ms)
            } else {
                poll_interval
            };
            // 空闲阈值同样支持配置实时生效；配置异常（0）时回退到启动参数。
            let idle_threshold = if cfg.idle_threshold_minutes > 0 {
                Duration::from_secs(cfg.idle_threshold_minutes.saturating_mul(60))
            } else {
                idle_threshold
            };
            ...
            if gap > poll_interval * 5 {
                ...
                let gap_start =
                    chrono::Utc::now() - chrono::Duration::from_std(gap).unwrap_or_default();
            ...
            if !is_paused {
                ...
                if now_idle_input && !is_idle {
                    ...
                    } else if !is_idle && let Some(fg) = foreground {
