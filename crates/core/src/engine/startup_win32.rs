...
impl StartupSource for RegistrySource {
    ...
    fn scan(&self) -> Vec<StartupEntryRecord> {
        ...
        let key = match winreg::RegKey::predef(self.predef())
            .open_subkey_with_flags(self.subkey, winreg::enums::KEY_READ)
        {
            Ok(k) => k,
            Err(e) => {
                warn!("Failed to open {}: {e}", self.source_name);
                return entries;
            }
        };
    ...
    fn disable(&self, entry: &StartupEntryRecord) -> Result<DisableResult, String> {
        ...
        let key = winreg::RegKey::predef(self.predef())
    ...
    fn enable(&self, entry: &StartupEntryRecord) -> Result<(), String> {
        ...
        let key = winreg::RegKey::predef(self.predef())
            .open_subkey_with_flags(self.subkey, winreg::enums::KEY_WRITE)
            .map_err(|e| format!("Cannot open key: {e}"))?;
    ...
    fn scan(&self) -> Vec<StartupEntryRecord> {
        ...
        let mut scan_dir = |dir: &PathBuf, enabled: bool| {
            if let Ok(rd) = std::fs::read_dir(dir) {
                for e in rd.flatten() {
                    let p = e.path();
                    if p.extension().is_some_and(|ext| ext == "lnk") {
                        let name = p
                            .file_stem()
                ...
                if parts.len() >= 3 {
                        ...
                        || trigger.contains("boot")
                    {
                        entries.push(StartupEntryRecord {
                            ...
                            enabled: !trigger.contains("disabled"),
                            backup_value: None,
                            backup_path: None,
                            first_seen: now,
                            last_checked: now,
...
impl Default for WindowsStartupScanner {
    fn default() -> Self {
        Self::new()
    }
}
