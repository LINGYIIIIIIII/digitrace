...
pub struct SessionAggregator {
    ...
    /// Exe-file names (e.g. "steam", "spotify") that must not be tracked.
    /// Time spent in excluded apps is not recorded at all.
    excluded: Vec<String>,
...
impl SessionAggregator {
    pub fn new(db: Arc<dyn DataStore>, excluded: Vec<String>) -> Self {
        Self {
            db,
            excluded,
    ...
    /// Match against the configured exclusion list: by exe file name
    /// (case-insensitive, without the ".exe" suffix) or by display name.
    fn is_excluded(&self, app: &AppInfo) -> bool {
        ...
        let stem = app
            ...
            .to_lowercase();
        self.excluded.iter().any(|raw| {
            let e = raw.trim().to_lowercase();
            !e.is_empty() && (e == stem || e == app.display_name.to_lowercase())
        })
    }
        ...
        match event {
            TrackedEvent::AppSwitched {
                ...
                if self.is_excluded(&current) {
                    debug!("App excluded, not tracked: {}", current.display_name);
            ...
            TrackedEvent::IdleEnded {
                ...
                if self.is_excluded(&current_app) {
                    debug!(
                        "Returned to excluded app, not tracked: {}",
                        current_app.display_name
                    );
                    return;
                }
...
mod tests {
    ...
    fn test_idle_session_recorded() {
        ...
        let mut agg = SessionAggregator::new(db.clone(), vec![]);
    ...
    #[test]
    fn test_excluded_app_not_recorded() {
        let db = Arc::new(MemoryStore::new());
        let mut agg = SessionAggregator::new(db.clone(), vec!["spotify".into()]);
        ...
        // Switch to the excluded app: its session must NOT be opened.
        let spotify = AppInfo::new(
            "C:/Program Files/Spotify/spotify.exe".into(),
            "spotify".into(),
        );
        agg.accept(TrackedEvent::AppSwitched {
            previous: Some(code),
        ...
        let sessions = db.get_sessions_by_date(chrono::Local::now().date_naive());
        assert!(
            sessions.iter().all(|s| s.app_name != "spotify"),
            "excluded app must not be recorded"
        );
    }
}
